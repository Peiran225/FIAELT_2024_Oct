...
from flearn.models.client_admm import Client_ADMM
...
class Server(object):
    def __init__(self, params, learner, dataset):
        ...
        self.z_model = [np.zeros_like(x) for x in self.latest_model]
        self.x_model = [np.zeros_like(x) for x in self.latest_model]
        self.new_y_i = [np.zeros_like(x) for x in self.latest_model]
    ...
    def setup_clients(self, dataset, train_model_list=None):
        ...
        all_clients = [Client_ADMM(u, g, train_data[u], test_data[u], 
    ...
    def train(self):
        ...
        print('Training using FedADMM with {} workers ---'.format(self.clients_per_round))
        ...
        for k in range(self.num_rounds):
            ...
            for idx, c in enumerate(self.clients):
                ...
                if idx in indices:
                    ...
                    soln_x, soln_z, stats, iterations = c.solve_inner(num_epochs=self.num_epochs, batch_size=self.batch_size,
                                                         update=True, new_y_i=self.new_y_i, local_optim=self.params['local_optim'], 
                                                         term_alpha=self.params['term_alpha'])
                    total_bytes_r += stats[2]
                    csolns_x.append(soln_x)
                    csolns_z.append(soln_z)
                    total_iterations.append(iterations)
                else:
                    soln_x, soln_z, stats = c.solve_inner(num_epochs=self.num_epochs, batch_size=self.batch_size, update=False)                    
                    csolns_x.append(soln_x)
                    csolns_z.append(soln_z)

            self.x_model = self.aggregate(csolns_x)
            self.z_model = self.aggregate(csolns_z)

            for i in range(len(self.latest_model)):
                self.new_y_i[i] = self.reg_function.prox_eval(self.x_model[i] + self.options['eta'] * self.z_model[i], 0.005 * self.options['eta']  )
                self.latest_model[i] = self.x_model[i]
